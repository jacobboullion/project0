...
object BookDao {
  ...
  def updateBookTitle(book : Book) : Boolean = {
    val conn = ConnectionUtil.getConnection()
    Using.Manager {use => 
      val stmt = use(conn.prepareStatement("UPDATE book SET title = ? where book_id = ?"))
      stmt.setString(1, book.title)
      stmt.setInt(2, book.book_id)
      stmt.execute()

    }.getOrElse(false)
  }

  def deleteBook(book : Book) : Boolean = {
    val conn = ConnectionUtil.getConnection()
    Using.Manager {use => 
      val stmt = use(conn.prepareStatement("DELETE FROM book WHERE book_id = ?"))
      stmt.setInt(1, book.book_id)
      stmt.execute()
