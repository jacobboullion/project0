package com.revature.wccli.cli


import scala.io.StdIn
import scala.util.matching.Regex
import com.revature.wccli.daos.BookDao
...
class Cli {
  ...
  def printOptions(): Unit = {
    ...
    println("update title : udates the title of given book_id")
    println("delete book : deletes book at given id")
  ...
  def menu(): Unit = {
    ...
    while (continueMenuLoop) {
      ...
      input match {
        ...
        case commandArgPattern(cmd, arg)
            if cmd.equalsIgnoreCase("update") && arg.equalsIgnoreCase("title") => {
              runUpdateBookSubMenu()
            }
        case commandArgPattern(cmd, arg)
            if cmd.equalsIgnoreCase("delete") && arg.equalsIgnoreCase("book") => {
  ...
  def runUpdateBookSubMenu(): Unit = {
    println("title?")
    val titleInput = StdIn.readLine()
    println("The book_id of the title you want to change?")
    val idInput = StdIn.readLine().toInt
    try {
      if (BookDao.updateBookTitle(Book(idInput, titleInput, "0000000000000"))) {
        println(s"updated title of book id = $idInput!")
      } 
    } catch {
      case e : Exception => {
        println("failed to update title.")
  ...
  def runDeleteBookSubMenu(): Unit = {
    println("The book_id of the book you want to delete?")
    val idInput = StdIn.readLine().toInt
    try {
      if (BookDao.deleteBook(Book(idInput, "", "0000000000000"))) {
        println(s"Deleted book with id = $idInput!")
      } 
    } catch {
      case e : Exception => {
        println("failed to update title.")
